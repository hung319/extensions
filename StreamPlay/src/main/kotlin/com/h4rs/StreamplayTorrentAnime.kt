...
import com.h4rs.StreamPlayAnime.LinkData
...
open class StreamplayTorrentAnime : MainAPI() {
    ...
    private suspend fun tmdbToAnimeId(title: String?, year: Int?, type: TvType): com.h4rs.AniIds {
        if (title.isNullOrBlank()) return com.h4rs.AniIds(null, null)
    ...
    """.trimIndent()
        ...
        val res = app.post(StreamPlay.anilistAPI, requestBody = data)
            .parsedSafe<com.h4rs.AniSearch>()
            ?.data
            ?.let { it.Page?.media ?: it.media }
            ?.firstOrNull()

        return com.h4rs.AniIds(res?.id, res?.idMal)
    }
