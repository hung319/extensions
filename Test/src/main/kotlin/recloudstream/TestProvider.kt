...
class NangCucProvider : MainAPI() {
    ...
    private fun extractSubtitlesFromHtmlAndCallback(document: Document, subtitleCallback: (SubtitleFile) -> Unit) {
                ...
                if (arrayContentMatch != null) {
                    ...
                    val scriptSubtitles = subObjectRegex.findAll(arrayContent).mapNotNull { objMatchResult ->
                        ...
                        if (extractedLang != null && subUrlValue != null && subUrlValue.startsWith("http")) {
                            // Sửa lỗi deprecated và lỗi No parameter name 'name'
                            val finalLangCode = if (extractedLang.equals("VI", ignoreCase = true)) "vi" else extractedLang.lowercase() // SỬA Ở ĐÂY
                            // Bỏ tham số name nếu SubtitleFile không hỗ trợ
                            SubtitleFile(lang = finalLangCode, url = subUrlValue) // SỬA Ở ĐÂY
                        } else { null }
                    }.toList()
        ...
        allExtractedSubtitles.distinctBy { it.url }.forEach { subtitle ->
            subtitleCallback(subtitle) // subtitle.name sẽ không còn nếu SubtitleFile không có
            subtitlesCalledCount++
        }
        println("NangCucProvider LOG (extractSubtitles): Đã gọi subtitleCallback $subtitlesCalledCount lần.")
    }
        ...
        try {
            ...
            if (newestSection != null) {
                ...
                val movies = newestSection.select("div.flw-item").mapNotNull { item ->
                    ...
                    val movieName = filmPoster?.selectFirst("img.film-poster-img")?.attr("title") // Đã định nghĩa movieName
                    ...
                    if (movieName != null && movieHref != null && moviePosterUrl != null) {
                        newMovieSearchResponse( // Dòng 123 (khoảng)
                            name = movieName, // movieName được truyền đúng
                            url = if (movieHref.startsWith("http")) movieHref else mainUrl + movieHref
                        ) {
                            this.posterUrl = moviePosterUrl
                            this.type = TvType.Movie
                            // Đảm bảo không có tham chiếu nào đến 'name' không xác định ở đây
                        }
                    } else { null }
                }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        try {
            ...
            val recommendationsList = document.select("section.block_area-related div.flw-item").mapNotNull {item ->
                ...
                val nameRec = filmPosterRec?.selectFirst("img.film-poster-img")?.attr("title") // Đổi tên biến để tránh nhầm lẫn
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            try {
                ...
                println("NangCucProvider LOG (loadLinks): Lỗi khi fetch/parse trang phim để lấy phụ đề: ${e.message}")
            }
