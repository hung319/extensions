...
class AnikotoProvider : MainAPI() {
    ...
    // JSON 1: List server (chứa HTML trong field result)
    data class AjaxResponse(
        val status: Int,
        val result: String
    )

    // JSON 2: Link embed (chứa URL trong result.url)
        ...
        doc.select(".servers .type li").forEach { server ->
            ...
            if (linkId.isNotBlank()) {
                ...
                try {
                    ...
                    // Kiểm tra sơ bộ tránh lỗi parse nếu server trả về HTML lạ
                    if (!responseText.trim().startsWith("<")) {
                        ...
                        if (!embedUrl.isNullOrBlank()) {
                            ...
                            // Cloudstream tự động xử lý link megaplay.buzz/vidstream/etc.
