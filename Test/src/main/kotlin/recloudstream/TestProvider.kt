...
class AnimeVietsubProvider : MainAPI() {
    ...
    private suspend fun getBaseUrl(): String {
        ...
        try {
            // Thêm interceptor Cloudflare vào yêu cầu lấy tên miền
    ...
    //----------------------------------------------------------------
    // CÁC HÀM API CHÍNH (search, getMainPage, load, loadLinks)
    //----------------------------------------------------------------
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            // Thêm interceptor Cloudflare cho yêu cầu AJAX
    ...
    private fun Document.toLoadResponse(
        ...
        try {
            ...
            val isMovie = this.determineFinalTvType(title, tags, episodes.size) == TvType.Movie

            return if (!isMovie) {
                provider.newTvSeriesLoadResponse(title, infoUrl, TvType.Anime, episodes) {
                    this.posterUrl = posterUrl; this.plot = plot; this.tags = tags; this.year = year
