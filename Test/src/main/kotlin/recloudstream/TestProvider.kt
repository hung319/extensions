 ...
 * - RE-IMPLEMENTED: `ssplay.net` extractor with a new, more robust multi-server logic as requested by the user.
 * - The new logic first tries to find all available servers (ST, SG, etc.) and generate links for each.
 * - The old JsUnpacker logic is kept as a fallback for maximum stability.
 */
        ...
        coroutineScope {
            sources.map { sourceUrl ->
                async {
                    ...
                    // ĐÃ KHÔI PHỤC VÀ NÂNG CẤP: Logic cho ssplay.net
                    else if ("ssplay.net" in sourceUrl) {
                        try {
                            val videoId = sourceUrl.substringAfter("/v/").substringBefore(".html").substringBefore("?")
                            val embedResponse = app.get(sourceUrl, referer = mainUrl)
                            val embedDocument = embedResponse.document
                            var linksFound = false

                            // Phương pháp 1: Tìm danh sách server và tạo link
                            val servers = embedDocument.select("ul.list-server-items a")
                            if (servers.isNotEmpty()) {
                                servers.forEach { serverElement ->
                                    val serverName = serverElement.text().trim()
                                    if (serverName.isNotBlank()) {
                                        val m3u8Url = "https://ssplay.net/$serverName/$videoId.html"
                                        callback.invoke(
                                            ExtractorLink("SSPlay", "SSPlay $serverName", m3u8Url, sourceUrl, Qualities.Unknown.value, type = ExtractorLinkType.M3U8)
                                        )
                                        linksFound = true
                                    }
                                }
                            ...
                            // Phương pháp 2 (Dự phòng): Nếu không có danh sách server, giải mã script
                            if (!linksFound) {
                                val packedRegex = Regex("""(eval\(function\(p,a,c,k,e,d\)\{.*sources.*?\}\\)\))""")
                                val packedScript = packedRegex.find(embedResponse.text)?.groupValues?.get(1)
