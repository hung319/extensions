...
class AnimeHayProvider : MainAPI() {
    ...
    private val domainCheckUrl = mainUrl // URL dùng để kiểm tra domain mới (thường là domain gốc .in)
    ...
    // =================================================================================
    // START: REFACTORED loadLinks AND HELPER FUNCTION
    // =================================================================================

    /**
     * Hàm trợ giúp để trích xuất link từ các server có cấu trúc .php?id={id}
     */
    private fun extractServerFromPhp(
        document: Document,
        ...
        iframeIdRegex: Regex,
        linkFormatter: (String) -> String, // Hàm để tạo link M3U8 từ ID
        callback: (ExtractorLink) -> Unit
    ): Boolean {
        var serverLink: String? = null
        var serverId: String? = null

        // 1. Tìm trong script và html trước
        val matchInScript = scriptRegex.find(combinedScript) ?: scriptRegex.find(pageHtml)
        if (matchInScript != null) {
            serverLink = matchInScript.groups[1]?.value?.let { fixUrl(it, baseUrl) }
            serverId = matchInScript.groups[2]?.value
        }

        // 2. Nếu không thấy, tìm trong iframe
        if (serverLink.isNullOrEmpty() || serverId.isNullOrEmpty()) {
            Log.d("AnimeHayProvider", "$serverName link not found in script/HTML, checking $iframeSelector...")
            val serverIframe = document.selectFirst(iframeSelector)
            serverLink = serverIframe?.attr("src")?.let { fixUrl(it, baseUrl) }
            if (!serverLink.isNullOrEmpty()) {
                serverId = iframeIdRegex.find(serverLink)?.groups?.get(1)?.value
                if (serverId.isNullOrEmpty()) serverLink = null // Vô hiệu hóa nếu không tìm thấy ID
            }
        }

        // 3. Xử lý và callback nếu tìm thấy
        if (!serverLink.isNullOrEmpty() && !serverId.isNullOrEmpty()) {
            Log.i("AnimeHayProvider", "Processing $serverName link ID: $serverId (from: $serverLink)")
            try {
                ...
                Log.e("AnimeHayProvider", "Error calling callback for $serverName M3U8", e)
            }
        } else {
            Log.w("AnimeHayProvider", "$serverName link/ID not found.")
        }
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            // 2. Server GUN (Sử dụng helper)
            val gunFound = extractServerFromPhp(
                document = document, combinedScript = combinedScript, pageHtml = pageHtml, baseUrl = baseUrl,
                serverName = "GUN",
                scriptRegex = Regex("""src=["'](https?://[^"']*gun\.php\?id=([^&"']+)&[^"']*)["']"""),
                iframeSelector = "iframe#gun_if[src*=gun.php]",
                iframeIdRegex = Regex(""".*gun\.php\?id=([^&"']+)"""),
                linkFormatter = { id -> "https://pt.rapovideo.xyz/playlist/v2/$id/master.m3u8" },
                callback = callback
            )
            if (gunFound) foundLinks = true
            
            // 3. Server PHO (Sử dụng helper)
            val phoFound = extractServerFromPhp(
                document = document, combinedScript = combinedScript, pageHtml = pageHtml, baseUrl = baseUrl,
                serverName = "PHO",
                scriptRegex = Regex("""src=["'](https?://[^"']*pho\.php\?id=([^&"']+)&[^"']*)["']"""),
                iframeSelector = "iframe#pho_if[src*=pho.php]",
                iframeIdRegex = Regex(""".*pho\.php\?id=([^&"']+)"""),
                linkFormatter = { id -> "https://pt.rapovideo.xyz/playlist/$id/master.m3u8" },
                callback = callback
            )
            if (phoFound) foundLinks = true


            // 4. Server HY (Hydrax - Logic riêng)
    ...
    // =================================================================================
    // END: REFACTORED loadLinks
    // =================================================================================


    // =================================================================================
    // START: UPDATED toSearchResponse to show episode info
    // =================================================================================
    ...
    private fun Element.toSearchResponse(provider: MainAPI, baseUrl: String): SearchResponse? {
        try {
            ...
            // *** NEW ***: Get episode info from the poster item
            val episodeInfo = this.selectFirst("div.info_right")?.text()?.trim()
            
            // *** NEW ***: Combine title with episode info for display
            val finalDisplayName = if (!episodeInfo.isNullOrBlank()) {
                "$title\n$episodeInfo" // Use newline for better formatting in UI
            } else {
                title
            }
            ...
            // Use finalDisplayName for the name property
            ...
            val episodes = this.select("div.list-item-episode a")?.mapNotNull { episodeLink ->
                ...
                if (!finalEpName.isNullOrEmpty() && !epUrl.isNullOrEmpty()) {
                    newEpisode(data = epUrl) {
                        ...
                        this.episode = null // Để Cloudstream tự xử lý
                    }
                } else { null }
            }?.reversed() ?: emptyList()
