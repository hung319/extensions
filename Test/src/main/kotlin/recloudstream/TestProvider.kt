...
class AnimeVietsubProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        var mainPageHasNext = false // This will be the hasNext for HomePageResponse

        when (page) {
            1 -> { // Load "Mới Cập Nhật"
                Log.d("AnimeVietsubProvider", "getMainPage: Loading Page 1 - Mới Cập Nhật")
                try {
                    ...
                    mainPageHasNext = true // Page 2 (Sắp Chiếu) exists
                } catch (e: Exception) {
                    Log.e("AnimeVietsubProvider", "Error loading /anime-moi/ for getMainPage page 1: ${e.message}", e)
                    mainPageHasNext = true // Still assume next page might exist even if this one fails
                }
            }
            2 -> { // Load "Sắp Chiếu"
                Log.d("AnimeVietsubProvider", "getMainPage: Loading Page 2 - Sắp Chiếu")
                try {
                    ...
                    mainPageHasNext = true // Page 3 (Đề cử) exists
                } catch (e: Exception) {
                    Log.e("AnimeVietsubProvider", "Error loading /anime-sap-chieu/ for getMainPage page 2: ${e.message}", e)
                    mainPageHasNext = true // Still assume next page might exist
                }
            }
            3 -> { // Load "Đề cử"
                Log.d("AnimeVietsubProvider", "getMainPage: Loading Page 3 - Đề cử")
                try {
                    val mainPageDoc = app.get(baseUrl).document
                    val hotItems = mainPageDoc.select("section#hot-home ul.MovieList.Rows li.TPostMv")
                    ...
                    mainPageHasNext = false // Assuming this is the last category for getMainPage
                } catch (e: Exception) {
                    Log.e("AnimeVietsubProvider", "Error loading #hot-home for getMainPage page 3: ${e.message}", e)
                    mainPageHasNext = false // If error, assume no more.
                }
            }
            else -> {
                // No more pages for getMainPage categories
                Log.d("AnimeVietsubProvider", "getMainPage: No more pages, requested page $page")
                return HomePageResponse(emptyList(), false)
            }
        }
        
        // If lists is empty for the current page, but mainPageHasNext was true (e.g. page 1 failed but page 2 exists)
        // we still return mainPageHasNext as true.
        // If lists is empty AND mainPageHasNext is false (e.g. page 3 failed, and it's the last), then it's correctly false.
        Log.d("AnimeVietsubProvider", "getMainPage: Returning page $page with ${lists.size} lists. HasNext: $mainPageHasNext")
        return HomePageResponse(lists, mainPageHasNext)
    }

    // ... (rest of your existing code: search, load, toSearchResponse, EpisodeData, getCountry, toLoadResponse, AjaxPlayerResponse, LinkSource, loadLinks, encodeUri, toAnimeVietsubRatingInt, fixUrl)
    // Make sure the rest of the functions are included here from your original/previous code.
    // For brevity, I'm omitting the identical parts.
    ...
    private fun Element.toSearchResponse(provider: MainAPI, baseUrl: String): SearchResponse? {
        return try {
            ...
            } else if (hasEpisodeSpan || isUpcoming) {
    ...
    private suspend fun Document.toLoadResponse(
        ...
        try {
            ...
            if (posterUrlForResponse.isNullOrBlank()) {
             ...
             if (posterUrlForResponse.isNullOrBlank()) {
                rawPosterUrl = infoDoc.selectFirst("div.TPost.Single div.Image img")?.attr("src") // This selector might be redundant if the one above is the same
                if (!rawPosterUrl.isNullOrBlank()) {
                    posterUrlForResponse = fixUrl(rawPosterUrl, baseUrl)
                }
            }
            ...
            val currentShowStatus = when {
                ...
                else -> if (episodesCount > 0 && parsedEpisodes.any { it.episode != null }) null else ShowStatus.Completed // Fallback, check if any episode has a number
            }
