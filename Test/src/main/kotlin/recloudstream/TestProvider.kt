...
// import android.util.Log // Bỏ comment nếu cần Log
...
class HHNinjaProvider : MainAPI() {
    ...
    override val supportedTypes = setOf(
        TvType.Cartoon // Đã đúng: Provider này hỗ trợ Cartoon
    )
    ...
    // --- HÀM toSearchResult ĐƯỢC CẬP NHẬT ---
    private fun Element.toSearchResult(): SearchResponse? {
        ...
        // Truyền TvType.Cartoon vào newAnimeSearchResponse
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val episodes = document.select("div.list_episode div.list-item-episode a").mapNotNull { el ->
            ...
            if (epName.contains("Trailer", ignoreCase = true) || epName.contains("PV", ignoreCase = true)) {
                ...
                // SỬ DỤNG newEpisode builder thay vì constructor cũ
                newEpisode(fixUrl(epHref)) {
                    this.name = epName
                    this.episode = epNumString?.replace(Regex("[^0-9]"), "")?.toIntOrNull()
                    // runTime không có sẵn từ trang web này cho mỗi tập,
                    // nên để trống, newEpisode sẽ xử lý giá trị mặc định (thường là null).
                }
            }
        }.reversed()

        // Xác định cấu trúc là series hay movie
        ...
        // Trả về LoadResponse với type là TvType.Cartoon
        return if (structuralTypeIsSeries) {
            newTvSeriesLoadResponse(title, url, TvType.Cartoon, episodes) { // <-- type là TvType.Cartoon
            ...
            newMovieLoadResponse(title, url, TvType.Cartoon, fixUrl(episodes.firstOrNull()?.data ?: "")) { // <-- type là TvType.Cartoon
                this.posterUrl = poster?.let { fixUrl(it) }
                this.plot = plot
                this.year = year
                this.recommendations = recommendations
