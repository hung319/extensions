...
class SettingsFragment(
    ...
    override fun onCreateView(
        ...
        val sortOptions = listOf("Seeders", "Qualitysize", "Quality", "Size", "QualityThenSeeders")
        sortSpinner.adapter =
        ...
        // ===== SUBTITLE LANGUAGE =====
        val subtitleLanguageTextView = root.findView<TextView>("subtitle_language_spinner")
        val subtitleLanguageOptions = metadataLanguageOptions
        val selectedSubtitleLanguages = BooleanArray(subtitleLanguageOptions.size)
        sharedPref.getString("subtitle_language", "")?.split(",")?.forEach { saved ->
            val index = subtitleLanguageOptions.indexOfFirst { it.first.equals(saved.trim(), ignoreCase = true) }
            if (index >= 0) selectedSubtitleLanguages[index] = true
        }

        val updateSubtitleLanguageText = {
            val selected = subtitleLanguageOptions.filterIndexed { index, _ -> selectedSubtitleLanguages[index] }
            subtitleLanguageTextView.text =
                if (selected.isEmpty()) "Select Subtitle Languages"
                else selected.joinToString(", ") { it.second }
        }
        updateSubtitleLanguageText()

        subtitleLanguageTextView.setOnClickListener {
            AlertDialog.Builder(requireContext())
                .setTitle("Select Subtitle Languages")
                .setMultiChoiceItems(
                    subtitleLanguageOptions.map { it.second }.toTypedArray(),
                    selectedSubtitleLanguages
                ) { _, which, isChecked ->
                    selectedSubtitleLanguages[which] = isChecked
                }
                .setPositiveButton("OK") { _, _ ->
                    updateSubtitleLanguageText()
                    sharedPref.edit {
                        putString(
                            ...
                            subtitleLanguageOptions
                                .filterIndexed { index, _ -> selectedSubtitleLanguages[index] }
        ...
        subtitleLanguageTextView.makeTvCompatible()

        // ===== SUBSENSE MANIFEST =====
        val subsenseManifestInput = root.findView<EditText>("subsense_manifest_input")
        subsenseManifestInput.setText(sharedPref.getString("subsense_manifest", ""))
        subsenseManifestInput.makeTvCompatible()
        ...
        val torrServerSyncCheckbox = root.findView<CheckBox>("torrserver_sync_checkbox")
        torrServerSyncCheckbox.isChecked = sharedPref.getBoolean("torrserver_use_fork", false)
        torrServerSyncCheckbox.makeTvCompatible()
        ...
        fun updateDebridKeyHint(provider: String) {
            ...
            torrServerSyncCheckbox.visibility = if (provider == "TorrServer") View.VISIBLE else View.GONE
        }
        ...
        saveBtn.setOnClickListener {
            sharedPref.edit {
                ...
                putString("subsense_manifest", subsenseManifestInput.text?.toString()?.trim().orEmpty())
                ...
                putBoolean("torrserver_use_fork", torrServerSyncCheckbox.isChecked)
            }
        ...
        resetBtn.setOnClickListener {
            AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton("Reset") { _, _ ->
                    ...
                    updateLanguageText()
                    selectedSubtitleLanguages.fill(false)
                    updateSubtitleLanguageText()
                    ...
                    subsenseManifestInput.text.clear()
                    torrServerSyncCheckbox.isChecked = false
                    restartApp()
                }
                .setNegativeButton("Cancel", null)
                .show()
