...
class TorraStream(private val sharedPref: SharedPreferences) : TmdbProvider() {
    ...
    override suspend fun loadLinks(
        ...
        val subsenseManifest = sharedPref.getString("subsense_manifest", "")?.trim().orEmpty()
        val subsenseBaseUrl = extractSubSenseBaseUrl(subsenseManifest)
        val subtitleLanguageCodes = sharedPref.getString("subtitle_language", "")
            ?.split(",")
            ?.mapNotNull { it.trim().lowercase().takeIf { trimmed -> trimmed.isNotEmpty() } }
            ?.toSet()
            .orEmpty()
        val subtitleLanguageFilter = subtitleLanguageCodes.takeIf { it.isNotEmpty() }
        val subsenseFilename = buildSubSenseFilename(title, year)
        val totalLinkLimit = sharedPref.getString("link_limit", "")?.trim()?.toIntOrNull() ?: 0
        val linkCounter = AtomicInteger(0)
        var linkScope: CoroutineScope? = null
        val limitedCallback: (ExtractorLink) -> Unit = if (totalLinkLimit > 0) {
            { link ->
                val count = linkCounter.incrementAndGet()
                if (count <= totalLinkLimit) {
                    callback(link)
                }
                if (count >= totalLinkLimit) {
                    linkScope?.cancel()
        ...
        val torrServerCategory = if (useTorrServerFork && season != null && episode != null) {
            buildTorrServerForkCategory(season, episode)
    ...
    private fun buildTorrServerStreamUrl(
        ...
        val params = mutableListOf<String>()
        if (useFork) {
            params += "link=${URLEncoder.encode(link, StandardCharsets.UTF_8.name())}"
            if (index != null) {
                params += "index=$index"
            }
        ...
        return if (useFork) {
            val safeFilename = filename?.takeIf { it.isNotBlank() } ?: "video.mkv"
            "$base/stream/$safeFilename?${params.joinToString("&")}"
        } else {
            "$base/stream?${params.joinToString("&")}"
        }
    }
    ...
    private fun toTorrServerLink(
        ...
        val streamUrl = buildTorrServerStreamUrl(
            baseUrl,
            linkParam,
            title,
            filename,
    ...
    private fun buildTorrServerFilename(
        title: String?,
        year: Int?,
        season: Int?,
        episode: Int?,
    ): String? {
        val safeTitle = title?.replace(Regex("[^A-Za-z0-9]+"), ".")?.trim('.')
        if (safeTitle.isNullOrBlank()) return null
        return if (season != null && episode != null) {
            String.format(Locale.US, "%s.S%02dE%02d.mkv", safeTitle, season, episode)
        } else if (year != null) {
            "$safeTitle.$year.mkv"
        } else {
            "$safeTitle.mkv"
    ...
    private fun buildTorrServerForkCategory(season: Int, episode: Int): String {
        return String.format(Locale.US, "tv-S%02dE%02d", season, episode)
    }

    private fun extractInfoHash(magnetUrl: String?): String? {
        if (magnetUrl.isNullOrBlank()) return null
        val lower = magnetUrl.lowercase()
        val idx = lower.indexOf("xt=urn:btih:")
        if (idx == -1) return null
        val start = idx + "xt=urn:btih:".length
        val end = lower.indexOf('&', start).let { if (it == -1) lower.length else it }
        val hash = magnetUrl.substring(start, end)
        return hash.takeIf { it.isNotBlank() }
    }

    private fun buildSubSenseFilename(title: String?, year: Int?): String? {
        val safeTitle = title?.replace(Regex("[^A-Za-z0-9]+"), ".")?.trim('.')
        if (safeTitle.isNullOrBlank()) return null
        val suffix = year?.let { ".$it" }.orEmpty()
        return "$safeTitle$suffix.mkv"
    }

    private fun extractSubSenseConfig(manifestUrl: String): String {
        return extractSubSenseBaseUrl(manifestUrl)
    }

    private fun extractSubSenseBaseUrl(manifestUrl: String): String {
